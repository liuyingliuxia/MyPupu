package me.vast.common.widgets.layoutline;

import android.content.Context;
import android.graphics.Canvas;
import android.support.annotation.ColorInt;
import android.support.annotation.Nullable;
import android.support.constraint.ConstraintLayout;
import android.util.AttributeSet;
import android.widget.FrameLayout;


/**
 * author : zhengz
 * time   : 2018/3/7
 * desc   : layout布局中可以上、下、左、右类型线条, 支持多种线条
 */

public class InsideLineConstraintLayout extends ConstraintLayout {
    private final BehaviorLine mBehaviorLine = BehaviorLine.create(this);

    public InsideLineConstraintLayout(Context context) {
        this(context, null);

    }

    public InsideLineConstraintLayout(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public InsideLineConstraintLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mBehaviorLine.init(attrs);
    }


    public void setLine(int lineGravity, float size, @ColorInt int lineColor) {
        mBehaviorLine.setLine(lineGravity, size, lineColor, 0, 0);
    }


    public void setLine(int lineGravity, float size, @ColorInt int lineColor, float start, float end) {
        mBehaviorLine.setLine(lineGravity, size, lineColor, start, end);
    }

    public void updateLine(int lineGravity, float size, @ColorInt int lineColor) {
        mBehaviorLine.updateLineColor(lineGravity, size, lineColor);
    }


    /**
     * 隐藏线
     *
     * @param lineGravity
     */
    public void hideGravity(int lineGravity) {
        mBehaviorLine.hideGravity(lineGravity);
    }

    /**
     * 显示线
     *
     * @param lineGravity
     */
    public void showGravity(int lineGravity) {
        mBehaviorLine.showGravity(lineGravity);
    }

    @Override
    public void dispatchDraw(Canvas canvas) {
        super.dispatchDraw(canvas);
        mBehaviorLine.drawLine(canvas);
    }
}
